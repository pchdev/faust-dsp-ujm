...
impl InteractiveWidget for Aliasing {
    fn on_key_event(&mut self, k: crossterm::event::KeyEvent) {
        match k.code {
            KeyCode::Enter => {
                self.aliased = !self.aliased; 
            }
            _ => ()
        }
    }
...
impl WidgetRef for Aliasing {
    fn render_ref(&self, area: Rect, buf: &mut ratatui::buffer::Buffer) {
        // High-resolution "continuous" waveform
        let continuous_points = 800;
        let continuous: Vec<(f64, f64)> = if !self.aliased {
            (0..=continuous_points).map(|i| {
            ...
            (0..=continuous_points).map(|i| {
                let x = i as f64 / continuous_points as f64;
                let y = (2.0 * std::f64::consts::PI * self.signal_freq / 4.0 * x).sin() * self.amplitude;
                (x, y)
            })
            .collect()
        };
